...
class PinYin
{
    ...
    private static $dict_list = array(
     ...
     * @return array
     */
    public static function getAllPy($chinese, $charset = 'utf-8')
    {
        if ($charset != 'gb2312') {
            $chinese = self::U2Utf8Gb($chinese);
        }

        $py = self::zhToPys($chinese);
     ...
     * @return string
     */
    public static function getFirstPy($all_pys, $delimiter = '')
    {
        if (count($all_pys) <= 0) {
            return '';
        }
        ...
        foreach ($all_pys as $one) {
            ...
            $result[] = strtoupper(substr($one, 0, 1));
        }
     ...
     * @return string
     */
    public static function getFirstLetter($all_pys)
    {
        if (count($all_pys) <= 0) {
            return '';
        }
    ...
    private static function zhToPy($num, $blank = '')
    {
        if ($num > 0 && $num < 160) {
            return chr($num);
    ...
    private static function zhToPys($chinese)
    {
        $result = array();
